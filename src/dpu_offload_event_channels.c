...
dpu_offload_status_t event_channel_register(dpu_offload_ev_sys_t *ev_sys, uint64_t type, notification_cb cb)
{
    ...
    ucs_list_for_each_safe(pending_notif, next_pending_notif, (&(ev_sys->pending_notifications)), item)
    {
        if (pending_notif->type == type)
        {
            ...
            cb((struct dpu_offload_ev_sys *)ev_sys, pending_notif->arg, (am_header_t *)pending_notif->header, pending_notif->header_size, pending_notif->data, pending_notif->data_size);
        }
    }
...
static dpu_offload_status_t op_completion_cb(struct dpu_offload_ev_sys *ev_sys, void *context, am_header_t *hdr, size_t hdr_len, void *data, size_t data_len)
{
...
static dpu_offload_status_t op_start_cb(struct dpu_offload_ev_sys *ev_sys, void *context, am_header_t *hdr, size_t hdr_len, void *data, size_t data_len)
{
    assert(context);
    assert(data);
    execution_context_t *econtext = (execution_context_t *)context;
...
static dpu_offload_status_t xgvmi_key_revoke_cb(struct dpu_offload_ev_sys *ev_sys, void *context, am_header_t *hdr, size_t hdr_len, void *data, size_t data_len)
{
    // todo
    return DO_ERROR;
}

static int xgvmi_key_recv_cb(struct dpu_offload_ev_sys *ev_sys, void *context, am_header_t *hdr, size_t hdr_len, void *data, size_t data_len)
{
    // todo
    return DO_ERROR;
}
...
static int peer_cache_entries_recv_cb(struct dpu_offload_ev_sys *ev_sys, void *context, am_header_t *hdr, size_t hdr_len, void *data, size_t data_len)
{
    assert(context);
    assert(data);
